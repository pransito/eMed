...
warning('MANNHEIM SPSS DOES HAS BOTH INFOS')
...
inv_bnn$ID = as.character(inv_bnn$Co.ID)
...
if (des_pred == 'relapse') {
  ...
  # Bonn
  relabs_bn$ADD = gsub('ADD_P_','',relabs_bn$ADD)
  relabs_bn$ADD = gsub('ADDP_P_','',relabs_bn$ADD)
  relabs_bn$ADD = gsub('^0','',relabs_bn$ADD)
  for (ii in 1:length(inv$relapse)) {
    if (is.na(inv$relapse[ii])) {
      cur_id  = inv$ID_initials_NGFN[ii]
      if (is.na(cur_id)) {
